...
extension Array where Element: Differentiable, Element.TangentVector == Element {
    ...
    public mutating func scatteringAdd<C>(
        ...
        assert(indices.count == values.count, "Mismatched indices and values length, \(indices.count) vs \(values.count)")
    ...
    public mutating func _vjpScatteringAdd<C>(
        ...
        return ((), { tv in
            ...
            if tv.base.isEmpty {
                return [Element].TangentVector(repeating: .zero, count: valuesCount)
            }
