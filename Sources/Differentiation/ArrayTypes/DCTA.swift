...
extension DCTA {
    ...
    @inlinable
    mutating func _vjpSubscriptSet(
        newValue: Element,
        index: Int
    ) -> (value: Void, pullback: (inout DCTA.TangentVector) -> (Element.TangentVector)) {
        self[index] = newValue
        return (
            value: (),
...
extension DCTA: AdditiveArithmetic where Element: AdditiveArithmetic {
    ...
    public static func - (lhs: DCTA<Element>, rhs: DCTA<Element>) -> DCTA<Element> {
        if rhs.storage.isEmpty { return lhs }
        if lhs.storage.isEmpty {
            return DCTA(rhs.storage.map { .zero - $0 })
        }
...
extension DCTA {
    ...
    public func _vjpDifferentiableMap<Result: Differentiable>(_ body: @differentiable(reverse) (Element) -> Result)
        -> (value: DCTA<Result>, pullback: (DCTA<Result>.TangentVector) -> DCTA.TangentVector)
    {
