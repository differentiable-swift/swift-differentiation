...
public extension ConstantTimeAccessor {
    ...
    static func + (lhs: Self, rhs: Self) -> Self {
        if lhs.values.isEmpty { return rhs }
    ...
    @inlinable
    static func - (lhs: Self, rhs: Self) -> Self {
        if lhs.values.isEmpty { return ConstantTimeAccessor(rhs.values.map { .zero - $0 }, accessed: .zero - rhs.accessed) }
        if rhs.values.isEmpty { return lhs }
...
extension ConstantTimeAccessor {
    ...
    public func differentiableMap<Result: Differentiable>(_ body: @differentiable(reverse) (Element) -> Result)
        -> ConstantTimeAccessor<Result>
    {
    ...
    @inlinable
    public func _vjpDifferentiableMap<Result: Differentiable>(_ body: @differentiable(reverse) (Element) -> Result)
        -> (
            value: ConstantTimeAccessor<Result>,
            pullback: (ConstantTimeAccessor<Result>.TangentVector) -> ConstantTimeAccessor.TangentVector
        )
    {
        ...
        return (
            ...
            pullback: { tangent in
                ...
                for pullback in pullbacks.reversed() {
                    ...
                    elementTangents.values
                        .append(elementTangent) // TODO: here we can do a prealloc and write in reverse to prevent the copy into the tangentvector at the end
                }
