...
extension DArray: AdditiveArithmetic where Element: AdditiveArithmetic {
    @inlinable
    public static var zero: DArray<Element> { .init([]) }
    ...
    public static func + (lhs: DArray<Element>, rhs: DArray<Element>) -> DArray<Element> {
        ...
        return DArray(zip(lhs.storage, rhs.storage).map(+))
    }

    @inlinable
    public static func - (lhs: DArray<Element>, rhs: DArray<Element>) -> DArray<Element> {
        if lhs.storage.count == 0 {
            return DArray(rhs.map { .zero - $0 })
        }
        if rhs.storage.count == 0 {
            return lhs
        }
...
extension DArray {
    ...
    @inlinable
    public func differentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (Element) -> Result) -> DArray<Result> {
        DArray<Result>(storage.map(transform))
    }
    ...
    internal func _vjpDifferentiableMap<Result: Differentiable>(
        ...
        return (
            value: values,
            pullback: { v in
                DArray.TangentVector(zip(v.storage, pullbacks).map { vi, pb in pb(vi) })
            }
        )
    }
