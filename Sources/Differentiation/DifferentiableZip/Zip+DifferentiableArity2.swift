...
@frozen
public struct Zip2SequenceDifferentiable<C1: Collection, C2: Collection> where C1.Index == Int, C2.Index == Int {@usableFromInline
internal var _collection1: C1
@usableFromInline
internal var _collection2: C2
    @inlinable
    internal init(_ collection1: C1,
_ collection2: C2    ) {self._collection1 = collection1
self._collection2 = collection2
    }
}

extension Zip2SequenceDifferentiable: Collection {
    public typealias Element = (C1.Element, C2.Element)
...
extension Zip2SequenceDifferentiable: Sendable where C1: Sendable,
C2: Sendable{}
...
@inlinable
public func _vjpDifferentiableZip<C1, C2>(_ collection1: C1,
_ collection2: C2
) -> (
    value: Zip2SequenceDifferentiable<C1, C2>,
    pullback: (Zip2SequenceDifferentiable<C1, C2>.TangentVector) -> (C1.TangentVector, C2.TangentVector)
) where
C1: Differentiable,
...
_collection2.move(by: offset.collection2)    }
    ...
    @inlinable
    public func differentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (C1.Element, C2.Element) -> Result
    ) -> [Result] {
        self.map(transform)
    }
    ...
    @inlinable
    public func _vjpDifferentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (C1.Element, C2.Element) -> Result
    ) -> (value: [Result], pullback: ([Result].TangentVector) -> TangentVector) {
        var results: [Result] = []
        results.reserveCapacity(self.count)
        var pullbacks: [(Result.TangentVector) -> (C1.Element.TangentVector, C2.Element.TangentVector)] = []
...
C2.TangentVector.Index == Int
    {
        ...
        public typealias Element = (C1.TangentVector.Element, C2.TangentVector.Element)
        public typealias Index = Int
...
var collection2: C2.TangentVector
        @inlinable
        init(_ collection1: C1.TangentVector, _ collection2: C2.TangentVector) {self.collection1 = collection1
...
@inlinable
public func differentiableZipWith<C1, C2, Result>(_ c1: C1,
_ c2: C2,    with transform: @differentiable(reverse) (C1.Element, C2.Element) -> Result
) -> [Result] where
C1: DifferentiableCollection,
C1.Element: Differentiable,
C2: DifferentiableCollection,
C2.Element: Differentiable,    Result: Differentiable
{
...
@inlinable
public func _vjpDifferentiableZipWith<C1, C2, Result>(_ c1: C1,
_ c2: C2,    with transform: @differentiable(reverse) (C1.Element, C2.Element) -> Result
) -> (value: [Result], pullback: ([Result].TangentVector) -> (C1.TangentVector, C2.TangentVector)) where
C1: DifferentiableCollection,
C1.Element: Differentiable,
C2: DifferentiableCollection,
C2.Element: Differentiable,
    Result: Differentiable
{
    ...
    if count == 0 {
        return (value: [], pullback: { v in (C1.TangentVector(), C2.TangentVector()) })
    }
    
    var results = ContiguousArray<Result>()
    results.reserveCapacity(count)
    var pullbacks: ContiguousArray<(Result.TangentVector) -> (C1.Element.TangentVector, C2.Element.TangentVector)> = []
