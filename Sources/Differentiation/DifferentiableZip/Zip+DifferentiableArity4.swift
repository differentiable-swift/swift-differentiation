...
extension Zip4SequenceDifferentiable: Collection {
    ...
    public var endIndex: Int {
        Swift.min(_collection1.count, _collection2.count, _collection3.count, _collection4.count)
    }
    
    @inlinable
    public subscript(index: Int) -> Element {
        (_collection1[index], _collection2[index], _collection3[index], _collection4[index])
    }
...
C4.TangentVector.Element == C4.Element.TangentVector{
    (
        ...
        pullback: { v in
            (v.collection1, v.collection2, v.collection3, v.collection4)
        }
    )
}
...
_collection4.move(by: offset.collection4)    }
    ...
    public func _vjpDifferentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (C1.Element, C2.Element, C3.Element, C4.Element) -> Result
        ...
        for parameters in self {
            let (value, pullback) = valueWithPullback(at: parameters.0, parameters.1, parameters.2, parameters.3, of: transform)
...
results4.reserveCapacity(v.count)
    ...
    for (tangentElement, pullback) in zip(v, pullbacks) {
        let (result1, result2, result3, result4) = pullback(tangentElement)
...
results4.appendContribution(of: result4)
                ...
                return TangentVector(results1, results2, results3, results4)
            }
        )
    }
}
...
C4.TangentVector.Index == Int
    {
        ...
        @inlinable
        public var endIndex: Int { 
            Swift.min(collection1.count, collection2.count, collection3.count, collection4.count)
        }
        
        @inlinable
        public subscript(index: Int) -> Element {
            (collection1[index], collection2[index], collection3[index], collection4[index])
        }
...
var c4i = c4.startIndex    
    for _ in 0 ..< capacity {
        results.append(transform(c1[c1i], c2[c2i], c3[c3i], c4[c4i]))
...
var c4i = c4.startIndex    
    for _ in 0 ..< count {
        let (value, pullback) = valueWithPullback(at: c1[c1i], c2[c2i], c3[c3i], c4[c4i], of: transform)
