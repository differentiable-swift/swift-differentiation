...
@frozen
public struct Zip3SequenceDifferentiable<C1: Collection, C2: Collection, C3: Collection> where C1.Index == Int, C2.Index == Int, C3.Index == Int {@usableFromInline
...
_ collection2: C2,
_ collection3: C3    ) {self._collection1 = collection1
...
extension Zip3SequenceDifferentiable: Collection {
    public typealias Element = (C1.Element, C2.Element, C3.Element)
...
extension Zip3SequenceDifferentiable: Sendable where C1: Sendable,
C2: Sendable,
C3: Sendable{}
...
@inlinable
public func _vjpDifferentiableZip<C1, C2, C3>(_ collection1: C1,
_ collection2: C2,
_ collection3: C3
) -> (
    value: Zip3SequenceDifferentiable<C1, C2, C3>,
    pullback: (Zip3SequenceDifferentiable<C1, C2, C3>.TangentVector) -> (C1.TangentVector, C2.TangentVector, C3.TangentVector)
) where
C1: Differentiable,
...
_collection3.move(by: offset.collection3)    }
    ...
    @inlinable
    public func differentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (C1.Element, C2.Element, C3.Element) -> Result
    ) -> [Result] {
        self.map(transform)
    }
    ...
    @inlinable
    public func _vjpDifferentiableMap<Result: Differentiable>(_ transform: @differentiable(reverse) (C1.Element, C2.Element, C3.Element) -> Result
    ) -> (value: [Result], pullback: ([Result].TangentVector) -> TangentVector) {
        var results: [Result] = []
        results.reserveCapacity(self.count)
        var pullbacks: [(Result.TangentVector) -> (C1.Element.TangentVector, C2.Element.TangentVector, C3.Element.TangentVector)] = []
...
C3.TangentVector.Index == Int
    {
        ...
        public typealias Element = (C1.TangentVector.Element, C2.TangentVector.Element, C3.TangentVector.Element)
        public typealias Index = Int
...
var collection3: C3.TangentVector
        @inlinable
        init(_ collection1: C1.TangentVector, _ collection2: C2.TangentVector, _ collection3: C3.TangentVector) {self.collection1 = collection1
...
@inlinable
public func differentiableZipWith<C1, C2, C3, Result>(_ c1: C1,
_ c2: C2,
_ c3: C3,    with transform: @differentiable(reverse) (C1.Element, C2.Element, C3.Element) -> Result
) -> [Result] where
...
C3.Element: Differentiable,    Result: Differentiable
{
...
@inlinable
public func _vjpDifferentiableZipWith<C1, C2, C3, Result>(_ c1: C1,
_ c2: C2,
_ c3: C3,    with transform: @differentiable(reverse) (C1.Element, C2.Element, C3.Element) -> Result
) -> (value: [Result], pullback: ([Result].TangentVector) -> (C1.TangentVector, C2.TangentVector, C3.TangentVector)) where
C1: DifferentiableCollection,
C1.Element: Differentiable,
C2: DifferentiableCollection,
C2.Element: Differentiable,
    ...
    Result: Differentiable
{
    ...
    if count == 0 {
        return (value: [], pullback: { v in (C1.TangentVector(), C2.TangentVector(), C3.TangentVector()) })
    }
    
    var results = ContiguousArray<Result>()
    results.reserveCapacity(count)
    var pullbacks: ContiguousArray<(Result.TangentVector) -> (C1.Element.TangentVector, C2.Element.TangentVector, C3.Element.TangentVector)> = []
